 ...
 * @Date: 2021-12-17 08:54:51 
 * @Last Modified by: Turtle (qq769711153@hotmail.com)
 * @Last Modified time: 2021-12-17 16:37:19
 */
#pragma once

// The number of records_ (and keys_) in a leaf node
constexpr int LEAF_MAXN = 3;
// The number of get_node_ptr (and keys_) in an internal node
constexpr int INTER_MAXN = 3;

constexpr int LEAF_MID_POS = (LEAF_MAXN  + 1) / 2;
constexpr int INTER_MID_POS = (INTER_MAXN) / 2;

constexpr int LEAF_MINN = (LEAF_MAXN  + 1) / 2;
constexpr int INTER_MINN = (INTER_MAXN  + 1) / 2;
...
template<typename T>
auto pretty_array(T* arr,int sz){
    std::string ans;
    ans.append("{");
    for(int i = 0; i < sz ; i++){
        ans.append(std::to_string(arr[i]));
...
template<int sz>
auto pretty_array(bp_node<int,char*>* (&arr)[sz]){
    std::string ans;
    ans.append("{");
    for(int i = 0; i < sz ; i++){
...
auto pretty_array(char** arr,int sz){
    std::string ans;
    ans.append("{");
    for(int i = 0; i < sz ; i++){
        ans.append(arr[i]);
