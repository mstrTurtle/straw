    ...
    double 时间增量)
{
    ...
    rtree.query(irq::intersect(矩形A) && !irq::within(矩形A),std::back_inserter(v));
    ...
    for(auto route: v){
        ...
        {
            auto wrapper = [&](bool in){return (起始状态 == 已经过A)?(!in):(in);};
            int& cntTarget = (起始状态 == 已经过A)?(cntC):(cntD);
            for(double 当前时间 = 起始时间; 当前时间 > route.time_beg; 当前时间 -= 时间增量){
                Coord2 coord = route.at<double>(当前时间);
                if(wrapper(inside(coord,矩形A))){
                    cntTarget++;
                }
            }
        }
        {
            auto wrapper = [&](bool in){return (终止状态 == 已经过A)?(!in):(in);};
